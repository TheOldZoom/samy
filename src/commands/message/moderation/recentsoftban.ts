...
export default new MessageCommand({
  name: "recentsoftban",
  description:
    "Chunk-softban members who joined recently (ideal for raid cleanup).",
  aliases: ["chunksoftban", "raidsoftban"],
  category: "Moderation",
  guildOnly: true,
  ...
  arguments: [
    ...
    },
    {
      ...
      aliases: ["r"],
      type: "string",
      description: "Reason for chunk-softbanning.",
      required: false,
      default: "Raid cleanup / recentsoftban",
    },
  ],
  ...
  async execute(client, message, args) {
    ...
    if (!thresholdInput) {
      await message.reply({
        ...
        components: [
          new Container().text(
            Text(
              `${icons.warning} ${client.i18n.t("commands.recentsoftban.missing_threshold")}`,
            ),
          ),
        ],
      });
    ...
    const reason = args.getString("reason") ?? "Raid cleanup / recentsoftban";
        ...
        batch.map(async (member) => {
          try {
            await message.guild!.members.ban(member.id, {
              ...
              reason: `${message.author.tag} (recentsoftban ${thresholdInput}): ${reason}`,
            });
    ...
    await createModerationCase({
      ...
      reason: `Chunk-softbanned ${successCount} members (${thresholdInput}): ${reason}`,
    });

    const description = durationMs
      ? client.i18n.t("commands.recentsoftban.complete_description_time", {
          duration: msToHuman(durationMs),
        })
      : client.i18n.t("commands.recentsoftban.complete_description_count", {
          count: candidates.length,
        });

    const failedLine =
      failCount > 0
        ? client.i18n.t("commands.recentsoftban.complete_failed", {
            failed: failCount,
          })
        : "";
